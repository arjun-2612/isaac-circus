...
import circus_extensions.tasks  # noqa: F401
from circus_extensions.simulation.optim import CMAESOptimizer
...
def main():
    ...
    project_root = f"{'/workspace' if os.path.exists('/workspace') else subprocess.check_output(['git', 'rev-parse', '--show-toplevel'], text=True).strip()}"
    data_file = project_root + "/source/circus_extensions/simulation/pace/data/" + env_cfg.sim2real.data_dir
