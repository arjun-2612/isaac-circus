...
class TerrainImporter:
    ...
    def __init__(self, cfg: TerrainImporterCfg):
        ...
        if self.cfg.terrain_type == "generator":
            ...
            # ------------------- EDGE MASK --------------------
            # 1. Reconstruct the 2D heightfield from the 3D mesh vertices
            vertices = terrain_generator.terrain_mesh.vertices

            x_coords = np.sort(np.unique(vertices[:, 0]))
            y_coords = np.sort(np.unique(vertices[:, 1]))
            num_cols = len(x_coords)
            num_rows = len(y_coords)

            height_field_np = np.zeros((num_rows, num_cols))
            col_indices = np.searchsorted(x_coords, vertices[:, 0])
            row_indices = np.searchsorted(y_coords, vertices[:, 1])
            height_field_np[row_indices, col_indices] = vertices[:, 2]

            # 2. Calculate the edge mask from the reconstructed heightfield
            height_field = torch.from_numpy(height_field_np).to(self.device)
            slope_threshold = 0.05
            grad_y = height_field[1:, :] - height_field[:-1, :]
            grad_x = height_field[:, 1:] - height_field[:, :-1]
            grad_y = torch.nn.functional.pad(grad_y, (0, 0, 0, 1), mode="constant", value=0)
            grad_x = torch.nn.functional.pad(grad_x, (0, 1, 0, 0), mode="constant", value=0)
            gradient_magnitude = torch.sqrt(grad_x**2 + grad_y**2)
            self.edge_mask = (gradient_magnitude > slope_threshold).long()
            # -----------------------------------------
