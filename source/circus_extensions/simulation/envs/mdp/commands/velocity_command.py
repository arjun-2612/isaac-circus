...
if TYPE_CHECKING:
    ...
    from .commands_cfg import UniformVelocityCommandCfg, Uniform3DVelocityCommandCfg, ProgressiveVelocityCommandCfg
...
class UniformVelocityCommand(CommandTerm):
    ...
    def heading_error(self) -> torch.Tensor:
        """The desired heading command in the world frame. Shape is (num_envs,)."""
    ...
    def _update_command(self):
        ...
        if self.cfg.heading_command:
            ...
            self.heading_command_b[env_ids] = heading_error
        ...
        self.heading_command_b[standing_env_ids] = 0.0
...
class Uniform3DVelocityCommand(CommandTerm):
    ...
    def __init__(self, cfg: Uniform3DVelocityCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
    ...
    def root_pos_w(self) -> torch.Tensor:
        """The desired base position in the world frame. Shape is (num_envs, 3)."""
    ...
    def root_quat_w(self) -> torch.Tensor:
        """The desired base orientation in the world frame as quaternion. Shape is (num_envs, 4)."""
        return torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device).expand(self.num_envs, 4)
    
    @property
    def root_lin_vel_w(self) -> torch.Tensor:
        """The desired base linear velocity in the world frame. Shape is (num_envs, 3)."""
        # convert base velocity to world frame
        return math_utils.quat_rotate(
            self.root_quat_w, self.vel_command_b[:, :3]
        )
    
    @property
    def root_ang_vel_w(self) -> torch.Tensor:
        """The desired base angular velocity in the world frame. Shape is (num_envs, 3)."""
        ang_vel_b = torch.zeros(self.num_envs, 3, device=self.device)
        ang_vel_b[:, 2] = self.vel_command_b[:, 3]
        # convert base velocity to world frame
        return math_utils.quat_rotate(
            self.root_quat_w, ang_vel_b
        )
    
    @property
    def root_lin_acc_w(self) -> torch.Tensor:
        """The desired base linear acceleration in the world frame. Shape is (num_envs, 3)."""
        return torch.zeros(self.num_envs, 3, device=self.device)
    
    @property
    def root_ang_acc_w(self) -> torch.Tensor:
        """The desired base angular acceleration in the world frame. Shape is (num_envs, 3)."""
        return torch.zeros(self.num_envs, 3, device=self.device)
    ...
    def _debug_vis_callback(self, event):
        ...
        base_pos_w[:, 2] += 0.5
        # -- resolve the scales and quaternions
        vel_des_arrow_scale, vel_des_arrow_quat = self._resolve_xy_velocity_to_arrow(self.command[:, :3])
        vel_arrow_scale, vel_arrow_quat = self._resolve_xy_velocity_to_arrow(self.robot.data.root_lin_vel_b[:, :3])
        # display markers
        self.goal_vel_visualizer.visualize(base_pos_w, vel_des_arrow_quat, vel_des_arrow_scale)
        self.current_vel_visualizer.visualize(base_pos_w, vel_arrow_quat, vel_arrow_scale)
    ...
    def _resolve_xy_velocity_to_arrow(self, xyz_velocity: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Converts the XY base velocity command to arrow direction rotation."""
        # obtain default scale of the marker
        default_scale = self.goal_vel_visualizer.cfg.markers["arrow"].scale
        # arrow-scale
...
class ProgressiveVelocityCommand(CommandTerm):
    r"""Command generator that generates a velocity command in SE(2) from uniform distribution.
    ...
    cfg: ProgressiveVelocityCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: ProgressiveVelocityCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
        ...
        self.is_heading_env = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.is_standing_env = torch.zeros_like(self.is_heading_env)
        self.scale_factor = torch.ones(self.num_envs, device=self.device)
        if self.cfg.use_curriculum:
            self.scale_factor = torch.zeros(self.num_envs, device=self.device)
        # -- metrics
        self.metrics["error_vel_xy"] = torch.zeros(self.num_envs, device=self.device)
        self.metrics["error_vel_yaw"] = torch.zeros(self.num_envs, device=self.device)
    ...
    def _update_command(self):
        ...
        self.vel_command_b[:, :2] *= self.scale_factor.unsqueeze(-1)
