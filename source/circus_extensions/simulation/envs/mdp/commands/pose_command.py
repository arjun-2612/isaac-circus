...
class TerrainBasedPose2dCommand(UniformPose2dCommand):
    ...
    def _update_metrics(self):
        ...
        self.metrics["position_error"] = torch.norm(self.robot.data.root_pos_w[:, :2] - self.pos_command_w[:, :2], dim=-1)
        self.metrics["heading_error"] = wrap_to_pi(self.robot.data.heading_w - self.heading_command_w).abs()
        ...
        settled = (
            ...
            & (base_speed < self.cfg.max_goal_speed)
        )
